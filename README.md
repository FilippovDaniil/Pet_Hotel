...
- **Клиент**: поиск номеров, бронирование с выбором услуг, заказ еды из буфета, просмотр счетов, чат с поддержкой
- **Ресепшн**: подтверждение броней, заселение/выселение, управление счетами
- **Администратор**: управление номерами, услугами, меню буфета, ответы клиентам в чате
...
```
  ...
  └──────────────────────────────────────────────────────────────────────┘
               ...
               └──► support-service  :8087   (чат поддержки)
...
http://localhost:8087/swagger-ui.html  → support-service
```
...
| api-gateway | `SUPPORT_SERVICE_URL` | `http://support-service:8087` |
...
| Метод | URL | Роль | Описание |
|---|---|---|---|
| `GET` | `/api/support/messages` | CUSTOMER | Вся переписка с поддержкой |
| `POST` | `/api/support/messages` | CUSTOMER | Отправить сообщение |
| `GET` | `/api/support/messages/unread-count` | CUSTOMER | Кол-во непрочитанных ответов |
| `POST` | `/api/support/messages/read` | CUSTOMER | Отметить ответы как прочитанные |
| `GET` | `/api/support/admin/conversations` | ADMIN | Список всех диалогов |
| `GET` | `/api/support/admin/conversations/{id}` | ADMIN | Диалог с конкретным клиентом |
...
4. API Gateway (JwtAuthFilter):
   ...
   - Если валидный: добавляет X-User-Id, X-User-Role, X-User-Email к запросу
...
| Чат с поддержкой | `/support` | CUSTOMER |
| Панель поддержки | `/support/admin` | ADMIN |
